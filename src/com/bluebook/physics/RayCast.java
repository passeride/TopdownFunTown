...
public class RayCast {
    ...
    public void updatePosition() {
        ray = new Line2D((float) source.getTransform().getGlobalPosition().getX(),
            ...
            (float) source.getTransform().getGlobalPosition().getX()
                + (float) Math.cos(angle) * max_distance,
            (float) source.getTransform().getGlobalPosition().getY()
    ...
    private float getRayCast(Line2D ray, Line2D collision) {
        return getRayCast(ray.x1, ray.y1, ray.x2, ray.y2, collision.x1, collision.y1, collision.x2,
            collision.y2);
    }

    float dist(float LineStartX, float LineStartY, float LineEndX, float LineEndY) {
        return (float) Math.sqrt(
            (LineEndX - LineStartX) * (LineEndX - LineStartX) + (LineEndY - LineStartY) * (LineEndY
                - LineStartY));
    }

    float getRayCast(float RayStartX, float RayStartY, float RayCosineDist, float RaySineDist,
        ...
        float s, t;
        s = (-RayEndY * (RayStartX - LineStartX) + RayEndX * (RayStartY - LineStartY)) / (
            -LineXDiff * RayEndY + RayEndX * LineYDiff);
        t = (LineXDiff * (RayStartY - LineStartY) - LineYDiff * (RayStartX - LineStartX)) / (
            -LineXDiff * RayEndY + RayEndX * LineYDiff);
