...
public class GraphicsRenderer {
    ...
    public void drawCircle(Vector2 position, double radius){
        double xOff = OrtographicCamera.main == null ? 0 : OrtographicCamera.main.getX();
        double yOff = OrtographicCamera.main == null ? 0 : OrtographicCamera.main.getY();

        gc.fillArc(position.getX() + xOff - radius, position.getY() + yOff - radius, radius * 2, radius * 2, 0, 360, ArcType.CHORD);
    }
    ...
    public void strokeCircle(Vector2 position, double radius){
        ...
        gc.strokeArc(position.getX() + xOff - radius, position.getY() + yOff - radius, radius * 2, radius  * 2, 0, 360, ArcType.CHORD);
    }
    ...
    public void drawBox(Vector2 position, double width, double height){
        ...
        gc.fillRect(position.getX() + xOff - width / 2, position.getY() + yOff - height / 2, width, height);
    }
    ...
    public void strokeBox(Vector2 position, double width, double height){
        ...
        gc.strokeRect(position.getX() + xOff - width / 2, position.getY() + yOff - height / 2, width, height);
    }
    ...
    public void strokeLine(Vector2 start, Vector2 dest){
        ...
        gc.strokeLine(start.getX() + xOff, start.getY() + yOff, dest.getX() + xOff, dest.getY() + yOff);
    }
